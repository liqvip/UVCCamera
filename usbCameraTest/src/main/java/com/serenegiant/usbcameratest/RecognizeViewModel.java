...
import cn.zhut.facelib.ui.callback.OnRegisterFinishedCallback;
import cn.zhut.facelib.ui.model.CompareResult;
import cn.zhut.facelib.ui.model.PreviewConfig;
import cn.zhut.facelib.util.ConfigUtil;
import cn.zhut.facelib.util.FaceRectTransformer;
import cn.zhut.facelib.util.face.FaceHelper;
import cn.zhut.facelib.util.face.RecognizeCallback;
import cn.zhut.facelib.util.face.constants.LivenessType;
import cn.zhut.facelib.util.face.constants.RecognizeColor;
import cn.zhut.facelib.util.face.constants.RequestFeatureStatus;
import cn.zhut.facelib.util.face.model.FacePreviewInfo;
import cn.zhut.facelib.util.face.model.PreviewSize;
import cn.zhut.facelib.util.face.model.RecognizeConfiguration;
...
public class RecognizeViewModel extends ViewModel implements RecognizeCallback {
    ...
    private void registerFace(final byte[] nv21, FacePreviewInfo facePreviewInfo) {
        ...
        registerNv21Disposable = Observable.create((ObservableOnSubscribe<Boolean>) emitter -> {
            ...
            int res = registerEngine.init(UsbCameraTestApplication.getApplication(), DetectMode.ASF_DETECT_MODE_IMAGE, DetectFaceOrientPriority.ASF_OP_0_ONLY,
                    1, FaceEngine.ASF_FACE_RECOGNITION);
            if (res == ErrorInfo.MOK) {
                boolean success = FaceServer.getInstance().registerNv21(UsbCameraTestApplication.getApplication(), nv21.clone(), previewSize.width,
     ...
     */
    public void init() {
        Context context = UsbCameraTestApplication.getApplication();
    ...
    public void destroy() {
        unInit();
        if (faceHelper != null) {
            ConfigUtil.setTrackedFaceCount(UsbCameraTestApplication.getApplication().getApplicationContext(), faceHelper.getTrackedFaceCount());
            faceHelper.release();
            faceHelper = null;
        }
...
/*    public void onRgbCameraOpened(Camera camera) {
    ...
    public void onRgbCameraOpened(PreviewSize size) {
        PreviewSize lastPreviewSize = previewSize;
        previewSize = size;
        // 切换相机的时候可能会导致预览尺寸发生变化
        initFaceHelper(lastPreviewSize);
    }
...
/*    public void onIrCameraOpened(Camera camera) {
    ...
    private void initFaceHelper(PreviewSize lastPreviewSize) {
        if (faceHelper == null || lastPreviewSize == null ||
            ...
            Context context = UsbCameraTestApplication.getApplication().getApplicationContext();
        ...
        if (faceHelper != null) {
            ...
            if (registerStatus == REGISTER_STATUS_READY && !facePreviewInfoList.isEmpty()) {
                ...
                if (facePreviewInfo.getMask() != MaskInfo.WORN) {
                    ...
                    Toast.makeText(UsbCameraTestApplication.getApplication(), "注册照要求不戴口罩", Toast.LENGTH_SHORT).show();
                    updateRegisterStatus(REGISTER_STATUS_DONE);
                }
            }
    ...
    public Point loadPreviewSize() {
        String[] size = ConfigUtil.getPreviewSize(UsbCameraTestApplication.getApplication()).split("x");
